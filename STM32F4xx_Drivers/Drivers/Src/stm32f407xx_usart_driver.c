 ...
 */
void USART_PeripheralControl(USART_reg *pUSARTx, uint8_t Cmd)
{
	if(Cmd == ENABLE)
	{
...
void USART_PeriClockControl(USART_reg *pUSARTx, uint8_t EnorDi)
{
	if(EnorDi == ENABLE)
	{
		if(pUSARTx == USART1)
		{
			USART1_PERI_CLOCK_ENABLE();
		}else if (pUSARTx == USART2)
		{
			USART2_PERI_CLOCK_ENABLE();
		}else if (pUSARTx == USART3)
		{
			USART3_PERI_CLOCK_ENABLE();
		}
		else if (pUSARTx == UART4)
		{
			UART4_PERI_CLOCK_ENABLE();
		}
	}
	else
	{
		//TODO
        if (pUSARTx == USART1) {
        	USART1_PERI_CLOCK_DISABLE();
        } else if(pUSARTx == USART2) {
        	USART2_PERI_CLOCK_DISABLE();
        } else if(pUSARTx == USART3) {
        	USART3_PERI_CLOCK_DISABLE();
        } else {
        	UART4_PERI_CLOCK_DISABLE();
        }
	}
 ...
 */
__weak void USART_ApplicationEventCallback(USART_Handle *pUSARTHandle,uint8_t event)
{
